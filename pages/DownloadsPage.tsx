...
const DownloadCard: React.FC<DownloadCardProps> = ({ icon, platform, description, cta }) => {
  const PlatformIcon = getIcon(icon);
  const CtaIcon = getIcon('DownloadIcon');
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300 p-8 flex flex-col items-center text-center transform hover:-translate-y-2">
      ...
      <a
        ...
        className="inline-flex items-center gap-2 justify-center w-full px-6 py-3 border border-transparent text-base font-medium rounded-lg text-white bg-primary hover:bg-secondary transition-all shadow-md hover:shadow-lg"
      >
